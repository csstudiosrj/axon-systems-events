    ...
    try {
      const [tRes, cRes, osRes, pRes] = await Promise.all([
        supabase
          ...
          .select(
            "*, clients(*), member:profiles!member_id(id, full_name, email, role, commission_percentage), service_orders(id, quote_id, quotes(id, title, salesperson_id))"
          )
      ...
      const quotesData = (osRes.data ?? []) as unknown as ApprovedQuote[];
      if (osRes.data) setApprovedQuotes(quotesData);
      if (cRes.data) setClients(cRes.data as Client[]);
      if (pRes.data) setTeam(pRes.data as unknown as Profile[]);

      // Merge manual: injeta quote nos transactions via quote_id
      if (tRes.data) {
                  ...
                  <tr key={t.id} className="hover:bg-background/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <select
                        ...
                        className={`text-xs rounded-full px-3 py-1.5 font-black uppercase border focus:outline-none cursor-pointer ${statusClass(t)}`}
                      >
              ...
              <div className="space-y-4">
                <div className="flex gap-2 p-1 bg-background rounded-lg border border-surface/50 w-fit">
                  <button
                    ...
                    className={`px-8 py-2 rounded text-xs font-black uppercase transition-all ${formType === "income" ? "bg-cs-green text-white shadow-lg" : "text-text-secondary"}`}
                  >
                    {L.receita}
                  </button>
                  <button
                    ...
                    className={`px-8 py-2 rounded text-xs font-black uppercase transition-all ${formType === "expense" ? "bg-red-500 text-white shadow-lg" : "text-text-secondary"}`}
                  >
                    {L.despesa}
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 bg-cs-green/10 text-cs-green border border-cs-green/20 py-3 rounded-md font-black text-xs uppercase hover:bg-cs-green/20 transition-all"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 bg-blue-500/10 text-blue-400 border border-blue-500/20 py-3 rounded-md font-black text-xs uppercase hover:bg-blue-500/20 transition-all disabled:opacity-50"
                    >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-surface border border-surface/50 py-3 rounded-md font-black text-xs uppercase text-white hover:bg-background transition-all"
                  >
                ...
                <div className="space-y-4">
                  ...
                  {selectedTransaction.attachment_url ? (
                    ...
                    >
                      ...
                      <p className="text-xs font-black text-white uppercase">Baixar Anexo</p>
                    </a>
                  ) : (
                    <p className="text-xs text-text-secondary italic uppercase">Nenhum documento anexado.</p>
                  )}
                </div>
              </div>
            </div>
