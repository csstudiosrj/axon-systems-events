...
import { 
  ...
  CheckCircle, XCircle, Save, Trash2, 
  Search, Building, Eye, 
  AlertTriangle, Upload, Download, 
  Receipt, Mail, MessageCircle, Users,
  TrendingUp, UserCheck, Percent
} from "lucide-react";
...
export default function FinanceiroPage() {
  ...
  // --- EXTRAÇÃO DE LABELS E CONFIGURAÇÕES ---
  const labels: Record<string, string> = systemPreferences?.custom_labels || {};
  const currency: string = systemPreferences?.currency_code || "BRL";
  const rawCategories = systemPreferences?.financial_categories;
  const categories: FinancialCategories = {
    income: (rawCategories?.income?.length ? rawCategories.income : DEFAULT_INCOME_CATEGORIES),
    expense: (rawCategories?.expense?.length ? rawCategories.expense : DEFAULT_EXPENSE_CATEGORIES),
  };
  ...
  const [formData, setForm] = useState({
    ...
    expense_type: "administrative" as "administrative" | "personnel" | "operational",
    category: "", 
    amount: "", status: "pending" as "pending" | "paid" | "cancelled",
    dueDate: "", paymentDate: "",
    ...
    try {
      const [tRes, cRes, osRes, pRes] = await Promise.all([
        supabase
          ...
          .select("*, clients(*), member:profiles!member_id(*), service_orders(id, quotes(id, title, salesperson_id))")
          .order("due_date", { ascending: true }),
        supabase.from("clients").select("*").order("company_name"),
        supabase.from("service_orders").select("id, quotes(id, title, salesperson_id)").order("created_at", { ascending: false }),
        supabase.from("profiles").select("id, full_name, email, role, commission_percentage").order("full_name")
      ]);
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    ...
    if (!error) {
      ...
      if (selectedTransaction?.id === id) {
        setSelectedTransaction(prev => prev ? { ...prev, status: newStatus as Transaction["status"], payment_date: payload.payment_date as string } : null);
      }
    }
  };
  ...
  const sendInvoiceEmail = async (t: Transaction) => {
    if (!t.clients?.email) {
      showToast("Cliente sem e-mail cadastrado.", "warning");
      return;
    }
  ...
  const isOverdue = (date: string, status: string) => 
    new Date(date + 'T00:00:00') < new Date(new Date().setHours(0,0,0,0)) && status === 'pending';
  ...
  const sendWhatsApp = (t: Transaction) => {
    ...
    const msg = `Olá, ${t.clients?.company_name}. Consta em nosso sistema um lançamento pendente: ${t.description} no valor de ${formatCurrency(t.amount)}, com vencimento em ${new Date(t.due_date + 'T00:00:00').toLocaleDateString('pt-BR')}. Poderia nos enviar o comprovante?`;
  ...
  const personnelStats = useMemo(() => {
    ...
    const totalSalaries = personnelTrans
      .filter(t => t.category.toLowerCase().includes('salário') || t.category.toLowerCase().includes('salario') || t.category.toLowerCase().includes('folha'))
      .reduce((a, b) => a + Number(b.amount), 0);
    const totalCommissions = personnelTrans
      .filter(t => t.category.toLowerCase().includes('comissão') || t.category.toLowerCase().includes('comissao'))
      .reduce((a, b) => a + Number(b.amount), 0);
  ...
  const clientSummary = useMemo(() => {
    return clients.map(c => {
      ...
      return { ...c, ltv, debt };
    }).filter(c => c.ltv > 0 || c.debt > 0).sort((a, b) => b.ltv - a.ltv);
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-surface/50">
              {loading ? (
                <tr><td colSpan={5} className="px-6 py-8 text-center"><Loader2 className="animate-spin mx-auto text-cs-green" size={24} /></td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-8 text-center text-text-secondary text-xs">Nenhum lançamento encontrado.</td></tr>
              ) : filtered.map((t) => (
  ...
  return (
    <div className="space-y-6 relative pb-12">
      ...
      {view === "list" && (
        <>
          <div className="flex gap-2 border-b border-surface/50 overflow-x-auto">
            {[
              ...
              <button 
                ...
                className={`px-8 py-4 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all flex items-center gap-2 whitespace-nowrap ${
                  ...
                  {[
                    ...
                    <div key={b.label} className="space-y-2">
                      <div className="h-2 w-full bg-background rounded-full overflow-hidden">
                        <div className={`h-full ${b.color} transition-all`} style={{ width: `${Math.min((b.val / (stats.pendingIn || 1)) * 100, 100)}%` }}></div>
                      </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-surface/50">
                  {clientSummary.length === 0 ? (
                    <tr><td colSpan={4} className="px-8 py-8 text-center text-text-secondary text-xs">Nenhum cliente com movimentações financeiras.</td></tr>
                  ) : clientSummary.map(c => (
                ...
                <div className="space-y-6">
                  <div className="flex gap-2 p-1 bg-background rounded-lg border border-surface/50 w-fit">
                    <button 
                      ...
                      className={`px-8 py-2 rounded text-[10px] font-black uppercase transition-all ${formData.type === 'income' ? 'bg-cs-green text-white shadow-lg' : 'text-text-secondary'}`}
                    >
                      Receita
                    </button>
                    <button 
                      ...
                      className={`px-8 py-2 rounded text-[10px] font-black uppercase transition-all ${formData.type === 'expense' ? 'bg-red-500 text-white shadow-lg' : 'text-text-secondary'}`}
                    >
                      Despesa
                    </button>
                  </div>
                  ...
                  {formData.type === 'expense' && (
                    <div className="flex gap-2">
                      {(['administrative', 'personnel', 'operational'] as const).map(t => (
                        <button 
                          key={t} 
                          type="button" 
                          onClick={() => setForm(prev => ({...prev, expense_type: t}))} 
                          className={`flex-1 py-2 rounded text-[9px] font-black uppercase border border-surface transition-all ${formData.expense_type === t ? 'bg-white text-black' : 'text-text-secondary'}`}
                        >
                          {t}
                        </button>
                      ))}
                  ...
                  {formData.status === 'paid' && (
                    <div>
                      <label className="block text-[10px] font-black text-text-secondary uppercase mb-1.5 tracking-widest">Data do Pagamento</label>
                      <input 
                        type="date" 
                  ...
                  <div>
                    ...
                    >
                      ...
                      {(formData.type === 'income' ? categories.income : categories.expense).map((cat: string) => (
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      onChange={e => setForm(prev => ({...prev, status: e.target.value as "pending" | "paid" | "cancelled"}))} 
                      className="w-full bg-background border border-surface rounded-md px-4 py-3 text-white text-sm focus:border-cs-green outline-none transition-all"
                    >
                      <option value="pending">Pendente</option>
                ...
                <div className="space-y-6">
                    ...
                    <div className="relative">
                      ...
                      <div className="relative">
                        ...
                        <input 
                          type="text" 
                          value={clientSearch} 
                          onChange={e => { setClientSearch(e.target.value); if (!e.target.value) setForm(prev => ({...prev, clientId: ''})); }} 
                        ...
                        <div className="absolute z-50 w-full mt-1 bg-surface border border-surface/50 rounded-md shadow-2xl">
                          {filteredClientsList.map(c => (
                            <button 
                              key={c.id} 
                              type="button" 
                              onClick={() => { setForm(prev => ({...prev, clientId: c.id})); setClientSearch(c.company_name); }} 
                      ...
                      {formData.clientId && (
                        <p className="text-[10px] text-cs-green mt-1 font-black">✓ Cliente vinculado</p>
                      )}
                    </div>
                  )}

                  {formData.expense_type === 'personnel' && formData.type === 'expense' && (
                    <div className="relative">
                      ...
                      <div className="relative">
                        ...
                        <input 
                          type="text" 
                          value={memberSearch} 
                          onChange={e => { setMemberSearch(e.target.value); if (!e.target.value) setForm(prev => ({...prev, memberId: ''})); }} 
                          className="w-full bg-background border border-surface rounded-md pl-9 pr-4 py-2.5 text-white text-sm focus:border-cs-green outline-none transition-all" 
                        ...
                        <div className="absolute z-50 w-full mt-1 bg-surface border border-surface/50 rounded-md shadow-2xl">
                          {filteredTeamList.map(m => (
                            <button 
                              key={m.id} 
                              type="button" 
                              onClick={() => { setForm(prev => ({...prev, memberId: m.id})); setMemberSearch(m.full_name); }} 
                      ...
                      {formData.memberId && (
                        <p className="text-[10px] text-cs-gold mt-1 font-black">✓ Colaborador vinculado</p>
                      )}
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      onChange={e => setForm(prev => ({...prev, serviceOrderId: e.target.value}))} 
                      className="w-full bg-background border border-surface rounded-md px-4 py-3 text-white text-sm focus:border-cs-green outline-none transition-all"
                    >
                      <option value="">Administrativo / Geral</option>
                      {serviceOrders.map(os => (
                        <option key={os.id} value={os.id}>OS: {os.quotes?.title || os.id.split('-')[0]}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      >
                        ...
                        {formData.attachmentUrl ? "Trocar" : "Upload"}
                      </button>
                      <input 
                        type="file" 
                        ref={fileInputRef} 
                      ...
                      {formData.attachmentUrl && (
                        <p className="text-[10px] text-cs-green mt-1 font-black truncate">✓ Arquivo anexado</p>
                      )}
                    </div>
                    <div>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase border ${
                    selectedTransaction.type === 'income' ? 'bg-cs-green/10 text-cs-green border-cs-green/20' : 'bg-red-500/10 text-red-500 border-red-500/20'
                  }`}>
              ...
              <div className="grid grid-cols-3 gap-6">
                ...
                <div className="bg-background p-4 rounded-lg border border-surface/50">
                  ...
                  <p className={`text-sm font-bold uppercase ${
                    selectedTransaction.status === 'paid' ? 'text-cs-green' :
                    selectedTransaction.status === 'cancelled' ? 'text-zinc-400' :
                    isOverdue(selectedTransaction.due_date, selectedTransaction.status) ? 'text-red-500' :
                    'text-cs-gold'
                  }`}>
                    {selectedTransaction.status === 'paid' ? 'Pago' : 
                     selectedTransaction.status === 'cancelled' ? 'Cancelado' : 'Pendente'}
                  </p>
                </div>
              </div>
