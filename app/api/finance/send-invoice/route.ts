...
// Inicialização do Resend utilizando o padrão de nomenclatura seguro (Sensitive)
...
export async function POST(request: Request) {
  try {
    ...
    if (!process.env.RESEND_API) {
      throw new Error('Configuracao de API de e-mail ausente no servidor.');
    }
    ...
    if (error || !t) {
      throw new Error('Lancamento financeiro nao localizado.');
    }
    ...
    // Link para o portal ou visualização da fatura
    const publicLink = `${process.env.NEXT_PUBLIC_SITE_URL}/financeiro/fatura/${t.id}`;
    ...
    const { error: mailError } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; color: #0d0807; max-width: 600px; margin: 0 auto; border: 1px solid #1a1413; padding: 40px; border-radius: 12px;">
          ...
          <p style="font-size: 16px; line-height: 1.6;">Ha um lancamento pendente no sistema ARXUM referente aos servicos prestados.</p>
          ...
          <p style="font-size: 10px; color: #a19d9c; text-align: center; margin-top: 40px;">E-mail automatico gerado pelo ecossistema ARXUM.</p>
        </div>
      `,
    });
    ...
    // Atualiza o rastreio de notificação
    await supabaseAdmin
      .from('financial_transactions')
    ...
    console.error("ERRO_API_COBRANCA:", error.message);
